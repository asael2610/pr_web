...
namespace Modelo
{
    public class Empleado
    {
        ...
        private DataTable drop_puesto()
        {
            ...
            MySqlDataAdapter query = new MySqlDataAdapter(strConsulta, conectar.conectar);
            query.Fill(tabla);
        ...
        public void drop_puesto(DropDownList drop)
        {
            drop.ClearSelection();
            drop.Items.Clear();
            drop.AppendDataBoundItems = true;  //limpia el control y el objeto logico que llena el control
        ...
        private DataTable grid_empleados()
        {
            ...
            conectar = new ConexionBD();
            conectar.AbrirConexion();
            String consulta = string.Format("select e.id_empleado as id,e.codigo,e.nombres,e.apellidos,e.direccion,e.telefono,p.puesto,e.fecha_nacimiento,p.id_puesto from empleados as e inner join puestos as p on e.id_puesto = p.id_puesto;\r\n");
        ...
        public int crear(string codigo, string nombres, string apellidos, string direccion, string telefono, string fecha, int id_puesto)
        {
            ...
            MySqlCommand query = new MySqlCommand(strConsulta, conectar.conectar);
